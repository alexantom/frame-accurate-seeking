...
function timecodeToFrames(timecode, frameRate, currentTimeSec) {
    var derivedframeRate = Math.round(frameRate);
    ...
    var totalFrames = hours * derivedframeRate * 3600 + minutes * derivedframeRate * 60 + seconds * derivedframeRate + frames;
    return totalFrames;
}

function getFileExtension(url) {
    const urlParts = url.split('?'); // Split the URL at the query parameter delimiter
    const filename = urlParts[0]; // Get the filename part
...
function determineSourceType(url) {
    ...
    switch (extension) {
        ...
        default:
            // Handle unknown formats or fallback to a default
            return 'progressive';
    }
}
...
var SmtpeController = /** @class */ (function () {
    ...
    SmtpeController.prototype.seekToSMPTE = function (smpteString) {
        ...
        try {
            ...
            console.log(smpte.toTime(), debugSmpte, targetTime, 'log', smpteString)
...
var SmpteTimestamp = /** @class */ (function () {
    ...
    SmpteTimestamp.fromTimeV2 = function (timestamp, assetDesc) {
        // to get to the start of the actual frame... use this
        var tmp = timestamp;
        var retVal = new SmpteTimestamp(null, assetDesc);
        retVal.hours = Math.floor(tmp / 3600);
    ...
    SmpteTimestamp.fromTimeWithAdjustments = function (timestamp, assetDesc) {
       // console.log('timestamp', timestamp,'assetDesc', assetDesc)
...
var videoConfig = {
    cdn: urlQueryParams.get('cdn'),
    videoUrl: urlQueryParams.get('videoUrl'),
    frameRate: Number(urlQueryParams.get('frameRate'))
}
...
if(videoConfig.cdn) {
    ...
    document.getElementById("video_url").value = videoConfig.videoUrl
    document.getElementById("frame_image").value = videoConfig.cdn
    document.getElementById("fps").value = Number(videoConfig.frameRate)
    videoMeta = videoConfig
}
console.log(videoMeta,'videoMeta')
var sourceVid = {}
var videoType = determineSourceType(videoMeta.videoUrl)
sourceVid[videoType] = videoMeta.videoUrl
...
var assets = [
   ...
   */
  {
    name: 'Sintel SMTPE timestamps, 25 FPS',
    source: sourceVid,
    frameRate: videoMeta.frameRate,
     // adjustmentFactor: 1001 / 1000
  },
];
...
var conf = {
  key: '317c7aa3-3d3c-4bfa-a1a0-06a694a56538',
...
var baseframeel = document.getElementById('baseframe');
var frame1el = document.getElementById('frame1');
var frame2el = document.getElementById('frame2');
var frame3el = document.getElementById('frame3');

function loadImg (videoMeta){
    //console.log(videoMeta,'-34-')
    const urlParams = new URLSearchParams(window.location.search);
    const frameRate = urlParams.get('frameRate');
    const cdn = urlParams.get('cdn');
    const videoUrl = urlParams.get('videoUrl');

    // console.log(frameRate, cdn, videoUrl)
    ...
    if (currentTimeDisplay.innerHTML !== String(currentTime)) {
        ...
        var frameNum = timecodeToFrames(currentTime, frameRate || videoMeta.frameRate, currentTimeSec)
        currentFrame.innerHTML = frameNum
        baseframeel.src = `${videoMeta.cdn}/frames/${frameNum}.jpg`;
        frame1el.src = `${videoMeta.cdn}/framed/1/${frameNum}.jpg`;
        frame2el.src = `${videoMeta.cdn}/framed/2/${frameNum}.jpg`;
        frame3el.src = `${videoMeta.cdn}/framed/3/${frameNum}.jpg`;
...
(function init() {
    ...
    submitFrameBtn = document.getElementById('submit_frame');
    ...
    submitFrameBtn.onclick = function() {
        console.log(player)
        var videoUrl = document.getElementById("video_url").value
        var frameImg = document.getElementById("frame_image").value
        var fps = document.getElementById("fps").value
        var frameRate = Number(fps)
        var sourceVid= {};
        var videoType = determineSourceType(videoUrl)
        sourceVid[videoType] = videoMeta.videoUrl
        var source = sourceVid;
        ...
        url.searchParams.set('cdn', frameImg);
        url.searchParams.set('videoUrl', videoUrl);
        url.searchParams.set('frameRate', frameRate);
  ...
  player.on('sourceloaded', function() {
    ...
    player.on('fullscreenexit', function() {
      ...
      player.on(bitmovin.player.PlayerEvent.TimeChanged, () => {
          ...
          console.log(bitmovin.player.PlayerEvent.TimeChanged, currentTime);
      });
      player.on(bitmovin.player.PlayerEvent.Seek, function(SeekEvent) {
        console.log('SeekEvent.position: ' , SeekEvent.position)
        console.log('SeekEvent.target: ' , SeekEvent.seekTarget)         
      });

      // Update frame info after seeking
      player.on(bitmovin.player.PlayerEvent.Seeked, () => {
          const currentTime = player.getCurrentTime();
          console.log(bitmovin.player.PlayerEvent.Seeked, currentTime);
          console.log('Seeked successfull')
         loadImg(videoMeta)
      });
  });
